...
class PROTOBUF_EXPORT SerialArena {
 ...
 private:
  ...
  // 减少空间使用量统计，处理无符号类型的减法逻辑
// 确保不会出现下溢（underflow）的情况
  void DelSpaceUsed(size_t space_used) {
      size_t current = space_used_.load(std::memory_order_relaxed);
      size_t new_value = current >= space_used ? current - space_used : 0;
      space_used_.store(new_value, std::memory_order_relaxed);
  }
